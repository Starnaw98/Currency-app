...
const initialState = {
  ...
  exact_currency: {},
  favorite_currencies:
    saved === null || saved.length === 0 ? [] : returnSavedFavorites(),
  filtered_currencies: [],
};
...
const reducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case "GET_EXACT_CURRENCY": {
      const exact_element = state.currencies.filter(
        (el) => el.currency === action.payload
      );
      return {
        ...state,
    ...
    case "GET_FAVORITES": {
      ...
      const fav_objects = state.currencies.filter(
